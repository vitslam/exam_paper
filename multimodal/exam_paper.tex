...
% ============ 试卷标题与说明 ============
\begin{center}
{\zihao{2}\heiti 大模型多模态 期末试卷}\\[0.3cm]
{\zihao{4}\kaishu 满分 150 分 \quad 考试时间 150 分钟}\\[0.2cm]
{\small 命题范围：视觉编码与 ViT / 对比学习与 CLIP / 模态融合与连接器 / 扩散生成 / 离散视觉 Token / 多模态位置编码 / 对齐与指令微调 / 系统效率与幻觉}
\end{center}
...
\qitem{9}{CLIP 实现 Zero-shot 图像分类时，对 $C$ 个类别通常构造 $C$ 个文本 prompt（如 ``a photo of a \{class\}''），预测类别为}{}
\fourchoices{与图像嵌入余弦相似度最大的文本嵌入所对应的类别}{图像嵌入与各类别 one-hot 向量点积最大者}{对 $C$ 个 prompt 的文本嵌入取平均后，与图像嵌入做 L2 距离最小者}{随机从 $C$ 个类别中采样，与 CLIP 无关}

\qitem{10}{Qwen-VL 等模型采用 M-RoPE（Multimodal Rotary Position Embedding）处理图文混合序列，其核心思想是}{}
\fourchoices{对文本 token 使用 1D 位置编码，对视觉 token 使用 2D 空间位置编码（行、列），并在注意力中分别施加旋转}{所有模态共享同一套绝对正弦位置编码，不区分文本与图像}{完全取消位置编码，依赖 CLIP 预训练权重隐式编码位置}{仅对 CLS token 施加 RoPE，patch token 不加位置信息}

\qitem{11}{关于视觉语言大模型中的对象幻觉（Object Hallucination），下列成因分析\textbf{最准确}的是}{}
\fourchoices{幻觉完全由解码温度过高导致，Greedy Search 可彻底消除}{语言先验过强、视觉特征注入不足、训练数据中图文对齐噪声等因素，使模型倾向于生成文本中常见但图像中不存在的对象}{幻觉仅发生在扩散模型中，自回归 VLM 不会产生幻觉}{增大视觉 token 数量一定会加剧幻觉，与语言模型容量无关}
...
\qitem{14}{CLIP 对称 InfoNCE 损失（图像到文本方向）可写为
\[
\mathcal{L}_{I\to T}=-\frac{1}{B}\sum_{i=1}^{B}\log\frac{\exp(\mathbf{I}_i^{\top}\mathbf{T}_i/\tau)}{\sum_{j=1}^{B}\exp(\mathbf{I}_i^{\top}\mathbf{T}_j/\tau)}.
\]
完整对称损失为 $\mathcal{L}_{\text{CLIP}}=$ \blank{5.5cm}。}{}
...
\noindent 某多模态推理服务部署单卡 A100-80GB，模型为 7B VLM（fp16），视觉编码器 ViT-L（$N_v^{\max}=576$），LLM 最大上下文 32k token。负载包括：（a）高分辨率文档 OCR（单图 $2048\times 2048$，AnyRes 切为 4 子图）；（b）长视频理解（30 fps，10 秒片段，每帧采样 1 次）；（c）在线图文对话（平均 2k 文本 token）。系统观察到：高分辨率请求导致 P99 延迟毛刺；视频任务频繁 OOM；并发提升后 GPU 利用率仅 40\%。
\begin{enumerate}[label=(\arabic*),leftmargin=2em,itemsep=0.2em,parsep=0pt,topsep=0pt]
  \item 分析现象（a）的根因，说明 AnyRes 切图后视觉 token 数量的计算方式，以及 token 合并（如 2$\times$2 pooling）如何缓解；（3 分）
  \item 针对现象（b），设计包含帧采样、视觉 token 压缩、跨帧 KV Cache 复用的方案，并量化 10 秒视频的视觉 token 总数（设每帧 $N_v=576$，压缩比 4:1）；（4 分）
  \item 针对现象（c），结合 Continuous Batching、视觉编码器批处理、Prefix Caching（复用 System Prompt 与共享图像 KV），给出至少 3 项优化方向及预期收益。（3 分）
\end{enumerate}
...
\noindent\textbf{9. A}\quad Zero-shot 分类：构造 $C$ 个 prompt，编码后与图像嵌入算余弦相似度，取最大者对应类别。无需训练分类头，体现 CLIP 的开放词汇能力。

\noindent\textbf{10. A}\quad M-RoPE 对文本用 1D 位置，对视觉 token 用 2D（$h,w$）位置，在 RoPE 旋转中分别处理，使模型感知空间结构。B、C、D 均不符合 Qwen-VL 等设计。

\noindent\textbf{11. B}\quad 对象幻觉成因：语言先验（"狗"常伴"草地"）过强、视觉注入不足、训练噪声、解码随机性等。A 错误：Greedy 仍可能幻觉；C 错误：VLM 普遍存在；D 错误：适量视觉 token 有助于 grounding，幻觉是多方因素。
...
\noindent\textbf{14.}\quad $\mathcal{L}_{\text{CLIP}}=\dfrac{1}{2}(\mathcal{L}_{I\to T}+\mathcal{L}_{T\to I})$，其中 $\mathcal{L}_{T\to I}$ 为文本到图像方向的 InfoNCE，形式与 $\mathcal{L}_{I\to T}$ 对称。
...
\subsubsection*{21. 扩散模型推导（10 分）}

\noindent\textbf{(1) $q(x_t|x_0)$ 闭式解（5 分）}

由 $x_t=\sqrt{\alpha_t}x_{t-1}+\sqrt{1-\alpha_t}\,\epsilon_{t-1}$ 递推，归纳得
\[
x_t=\sqrt{\bar\alpha_t}\,x_0+\sqrt{1-\bar\alpha_t}\,\bar\epsilon,\quad \bar\epsilon\sim\mathcal{N}(0,I),
\]
即 $q(x_t|x_0)=\mathcal{N}(\sqrt{\bar\alpha_t}\,x_0,(1-\bar\alpha_t)I)$。

\noindent\textbf{(2) 简化为噪声预测（5 分）}

原始 ELBO 含多步 KL 项。在 $x_t=\sqrt{\bar\alpha_t}x_0+\sqrt{1-\bar\alpha_t}\epsilon$ 下，$x_0$ 与 $\epsilon$ 一一对应。预测 $x_0$ 等价于预测 $\epsilon$，Ho 等证明加权变分下界可简化为 $\mathbb{E}[\|\epsilon-\epsilon_\theta(x_t,t)\|^2]$，即直接学习噪声，训练更稳定且效果相当。
...
\subsubsection*{24. 压轴题：多模态推理系统优化（10 分）}

\noindent\textbf{(1) 高分辨率延迟毛刺（3 分）}
...
\noindent\textbf{(2) 长视频 OOM 方案（4 分）}

10 秒 30 fps 采样 300 帧。每帧 576 token，压缩比 4:1 后每帧 144 token，总视觉 token $=300\times 144=43\,200$。方案：帧采样（如 1 fps 得 10 帧）、Perceiver 压缩、跨帧 KV 复用（相邻帧共享前缀）。优化后 10 帧 $\times$ 144 $=1440$ token，可纳入 32k 上下文。

\noindent\textbf{(3) 并发与利用率（3 分）}
