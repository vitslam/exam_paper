...
% ============ 试卷标题与说明 ============
\begin{center}
{\zihao{2}\CJKfamily{SimHei}\bfseries 大模型应用开发（AGENT+RAG）期末试卷}\\[0.3cm]
...
\qitem{1}{在 RAG 系统中，将长文档分割成若干块（Chunk）时，下列哪种做法最可能导致检索结果丢失跨块语义信息？}{}
\fourchoices{固定字符数切分，无重叠}{基于段落边界切分，保留少量重叠}{利用句子嵌入相似度进行语义切分}{按 Token 数均匀切分，重叠比例为 20\%}

\qitem{2}{某 Agent 收到指令「查询北京今天天气，然后帮我把结果发送到邮箱」。它依次调用了 \texttt{get\_weather(city)} 和 \texttt{send\_email(content, address)} 两个工具。这种执行模式属于：}{}
\fourchoices{单步 ReAct}{顺序依赖的 Plan-and-Execute}{并行工具调用}{动态 Re-planning}

\qitem{3}{关于 Embedding 模型在 RAG 中的角色，下列说法\textbf{错误}的是：}{}
\fourchoices{将文本映射到低维稠密向量空间}{用于计算查询与文档块的语义相似度}{必须与 LLM 使用相同的分词器}{不同领域任务可能需要不同的微调 Embedding 模型}

\qitem{4}{在 Agent 的记忆机制中，用于存储整个会话历史摘要的结构称为：}{}
\fourchoicestwo{Working Memory}{Episodic Memory}{Semantic Memory}{Procedural Memory}

\qitem{5}{假设你使用 BM25（稀疏检索）与 Dense Retrieval（稠密检索）进行混合检索（Hybrid Search），加权融合分数公式为 $\mathrm{score}=\alpha\cdot S_{\mathrm{dense}}+(1-\alpha)\cdot S_{\mathrm{bm25}}$。若希望优先保证高召回率，$\alpha$ 取值应倾向于：}{}
\fourchoicestwo{接近 0}{接近 1}{0.5}{无法确定}
...
\qitem{16}{BM25 打分公式中，逆文档频率项 $\mathrm{IDF}(q_i)=\ln\!\left(\dfrac{N-n(q_i)+0.5}{n(q_i)+0.5}+1\right)$，其中 $n(q_i)$ 为包含词 $q_i$ 的文档数。若语料 $N=1000$，某查询词出现在 $n(q_i)=10$ 篇文档中，则 $\mathrm{IDF}(q_i)=$ \blank{3cm}（用 $\ln$ 式表示）。}{}
...
\qitem{18}{混合检索中，Reciprocal Rank Fusion（RRF）将稠密检索排名 $r_d$ 与稀疏检索排名 $r_s$ 融合，标准公式为 $\mathrm{RRF}(d)=\dfrac{1}{k+r_d}+\dfrac{1}{k+r_s}$。若取 $k=10$，某文档在稠密检索中排名第 $1$、稀疏检索中排名第 $2$，则其 RRF 得分为 \blank{3cm}（保留分数形式或两位小数）。}{}
...
\fouranswerrow{1}{A}{2}{B}{3}{C}{4}{B}
\fouranswerrow{5}{B}{6}{B}{7}{B}{8}{B}
\fouranswerrow{9}{B}{10}{B}{11}{B}{12}{B}

\vspace{0.3cm}

\noindent\textbf{1. A}\quad 固定字符数切分且无重叠，极易在句子、段落甚至实体名称中间强行截断，跨块语义关联完全断裂，且相邻块之间无任何冗余上下文可恢复。B/C/D 均通过边界感知或重叠机制保留了一定程度的跨块语义连续性。故选 A。

\noindent\textbf{2. B}\quad 任务可分解为「先查天气 $\to$ 再发邮件」两个有先后依赖的子步骤，\texttt{send\_email} 的 \texttt{content} 依赖 \texttt{get\_weather} 的返回结果，属于按预定计划顺序执行的 Plan-and-Execute 模式。A 错误：涉及两次工具调用，非单步 ReAct；C 错误：两步存在依赖，无法并行；D 错误：无中途重规划迹象。故选 B。

\noindent\textbf{3. C}\quad Embedding 模型（如 BGE、E5、text-embedding-3）通常拥有独立于 LLM 的分词器与编码架构，RAG 流水线中二者解耦部署是常态。A/B/D 均为 Embedding 在 RAG 中的正确描述。故选 C。

\noindent\textbf{4. B}\quad Episodic Memory（情节记忆）用于存储智能体与用户交互的具体经历，包括完整或\textbf{摘要化}的会话历史，保留时间线与事件脉络。Working Memory 为当前活跃上下文；Semantic Memory 存储从经历中抽象出的事实与知识；Procedural Memory 存储技能与操作流程。故选 B。

\noindent\textbf{5. B}\quad 在多数语义检索场景下，稠密检索对同义表达、语义近邻的召回能力优于 BM25，提高 $\alpha$（接近 1）可加大稠密检索在融合分数中的权重，有利于提升整体召回率。$\alpha$ 接近 0 则偏向词汇匹配，可能漏掉语义相关但用词不同的文档。故选 B。
...
\noindent\textbf{16.}\quad $\ln\!\left(\dfrac{990.5}{10.5}+1\right)$（或等价地 $\ln\!\left(\dfrac{2002}{21}\right)$）\quad 代入 $N=1000$，$n(q_i)=10$：$\mathrm{IDF}=\ln\!\left(\dfrac{1000-10+0.5}{10+0.5}+1\right)=\ln\!\left(\dfrac{990.5}{10.5}+1\right)$。

\noindent\textbf{17.}\quad $20$\quad 每步 $2+0.5=2.5$s，8 步共 $8\times2.5=20$s。

\noindent\textbf{18.}\quad $\dfrac{23}{132}$（或 $\approx 0.17$）\quad $\mathrm{RRF}=\dfrac{1}{10+1}+\dfrac{1}{10+2}=\dfrac{1}{11}+\dfrac{1}{12}=\dfrac{12+11}{132}=\dfrac{23}{132}$。
