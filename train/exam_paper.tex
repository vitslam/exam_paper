...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small 大模型训练与并行计算 期末试卷}
...
% ============ 试卷标题与说明 ============
\begin{center}
{\zihao{2}\CJKfamily{SimHei}\bfseries 大模型训练与并行计算 期末试卷}\\[0.3cm]
...
\qitem{1}{在大模型混合精度（Mixed-Precision）训练中，使用 AdamW 优化器并维护 FP32 版本的 Master Weights。对于参数量为 $\Phi$ 的模型，仅存储模型参数、梯度和优化器状态（不含激活值与通信缓存），每个参数平均需要占用的显存大小为}{}
\fourchoices{$12\Phi$ 字节}{$16\Phi$ 字节}{$18\Phi$ 字节}{$20\Phi$ 字节}
...
\fourchoices{$\dfrac{P-1}{M+P-1}$}{$\dfrac{P-1}{M}$}{$\dfrac{M-1}{M+P-1}$}{$\dfrac{P}{M}$}
...
\fourchoices{$0.25 T_f$}{$0.5 T_f$}{$0.75 T_f$}{$1.5 T_f$}
...
\qitem{8}{在使用分布式数据并行（DDP）训练时，各节点需要对梯度进行同步。若集群有 $P$ 个 GPU 节点，梯度数据总大小为 $V$ 字节，采用环形全归约（Ring-AllReduce）算法。在每个训练步的梯度同步阶段，每个 GPU 发送的总数据量为}{}
\fourchoices{$V$ 字节}{$\dfrac{2(P-1)V}{P}$ 字节}{$\dfrac{(P-1)V}{P}$ 字节}{$2PV$ 字节}
...
{\small 命题：大模型训练与并行计算课程组 \quad 校对：教研组}
\end{center}
\vspace{0.3cm}
\noindent\rule{\linewidth}{0.6pt}
...
\vspace{0.3cm}
\noindent\textbf{1. B}\quad 在混合精度训练中：模型权重（FP16/BF16）$2\Phi$ 字节；梯度（FP16/BF16）$2\Phi$ 字节；AdamW 优化器状态包含 FP32 的 Master Weight（$4\Phi$）、一阶动量 $m$（$4\Phi$）、二阶动量 $v$（$4\Phi$），共 $12\Phi$ 字节。模型状态总显存为 $2\Phi+2\Phi+12\Phi=16\Phi$ 字节。故选 B。
...
\noindent\textbf{14.}\quad $112$\quad 由第 1 题知每个参数占用模型状态显存 16 字节，7B 模型最小模型状态显存 $M=7\times10^{9}\times16=112\times10^{9}$ 字节 $=112$ GB。
